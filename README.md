...
## v1.29 — Danh sách game gộp 1 nơi + sửa lỗi giật lặp lại giữa trận
**Giao diện:** bỏ 3 nút cứng (Free Fire/Liên Quân/"game khác") theo yêu cầu, gộp chung vào
đúng 1 danh sách tự phát hiện (bao gồm cả Free Fire/Liên Quân) - chức năng bấm BOOST vẫn y
hệt cũ, chỉ khác cách trình bày. Sửa luôn lỗi thật: app bàn phím "Laban Key" bị nhận nhầm
thành game (field category của Android có thể mặc định trùng giá trị CATEGORY_GAME ở app
không khai báo rõ loại) - giờ lọc chéo qua danh sách IME thật đang cài để loại bàn phím ra.

**Lỗi giật lặp lại giữa trận (khả năng cao là nguyên nhân chính bạn báo):** nhánh xử lý khi
RAM áp lực trước đây gọi dọn dẹp NGAY MỖI LẦN phát hiện, không có giới hạn tần suất như
nhánh "máy đang yên tĩnh". Nếu RAM cứ ở mức cao liên tục (thường gặp với game nặng/máy yếu),
việc dọn dẹp - gồm nhiều lệnh shell, có cả vòng lặp gọi lệnh riêng cho TỪNG app đang chạy
không giới hạn số lượng - có thể lặp lại mỗi 12-20 giây LIÊN TỤC suốt đoạn RAM cao, mỗi lần
đều tốn CPU thật. Đã sửa: thêm giới hạn tối thiểu 60 giây giữa 2 lần dọn dù RAM vẫn đang áp
lực (vẫn đủ nhanh để xử lý thật sự khẩn cấp), và giới hạn tối đa 10 app xử lý mỗi lần gọi.

## v1.27 — Giao diện nhiều tab (thanh điều hướng dưới cùng) + logo mới + game tự phát hiện
Tổ chức lại giao diện 1-màn-cuộn-dài trước đây thành 4 tab thật (Tổng quan / Game / Dọn dẹp /
Hệ thống) với thanh điều hướng ở dưới cùng - KHÔNG thêm toggle/tính năng giả nào để lấp đầy
tab, chỉ nhóm lại đúng nội dung đã có. Cụ thể:
- **Tab Game**: ngoài Free Fire/Liên Quân, giờ TỰ PHÁT HIỆN các game khác đã cài trên máy
  qua `PackageManager` (dựa vào `ApplicationInfo.category == CATEGORY_GAME` - cờ chính thức
  Android mà game khai báo để hệ thống áp dụng đúng Game Mode, cộng vài tiền tố package dự
  phòng cho game lớn phổ biến), hiện icon + tên thật, bấm "BOOST" là chạy đúng luồng tối ưu
  + mở game đã có sẵn - không phải danh sách cứng lập trình tay.
- **Tab Dọn dẹp**: thêm vòng tiến trình tròn (dùng lại `GaugeRingView` đã có sẵn cho HUD,
  không tạo view mới) hiện % THẬT theo từng bước đã hoàn thành (xoá cache app → dừng app
  nền → dọn cache toàn hệ thống) - không phải hoạt ảnh chạy giả cho đẹp.
- **Logo mới**: xử lý ảnh logo bạn gửi thành adaptive icon chuẩn Android (icon co giãn theo
  từng launcher - tròn/vuông tròn/vuông tuỳ máy), sinh đủ 5 mức độ phân giải (mdpi→xxxhdpi).

## v1.26 — Khảo sát 1 app thương mại cùng lĩnh vực (chỉ ở mức permissions công khai)
Kiểm tra Manifest (thông tin công khai, không giải mã ngược code bên trong - vì là app
thương mại đóng mã nguồn) của 1 app "game tweak" khác. Phát hiện: VPN nội bộ, web server
chạy ngầm trong app, tích hợp Telegram live chat, SDK quảng cáo Vungle, và quyền
`RECORD_AUDIO` không rõ lý do cho 1 app tối ưu hệ thống. **Đây là hướng đi KHÔNG theo** -
web server ngầm và quyền ghi âm không giải thích được là rủi ro thật (tăng diện tấn công,
thu thập dữ liệu không cần thiết).

**Học được đúng 1 kỹ thuật hợp lý:** dùng Shizuku tự cấp quyền Usage Access cho chính mình
qua lệnh `appops set <pkg> GET_USAGE_STATS allow`, thay vì luôn bắt người dùng tự vào Cài
đặt tìm và bật tay - đây là bước dễ gây bỏ cuộc/cấu hình thiếu nhất trong toàn bộ setup (và
đúng loại "quên cấp quyền" đã gây ra hiện tượng lag lại ở v1.19). Chỉ mở màn Cài đặt thủ
công khi chưa có Shizuku hoặc tự cấp thất bại.


## v1.25 — Rà soát ổn định hoá toàn diện (không thêm tính năng mới)
**Lỗi ổn định nghiêm trọng nhất tìm được:** toàn bộ logic xử lý mỗi lần đo trong
`HudOverlayService` (200+ dòng, chạy mỗi 12-20 giây suốt trận) trước đây KHÔNG có lớp bảo
vệ try/catch tổng - chỉ đúng bước đo số liệu (`adaptiveEngine.sample()`) được bọc an toàn.
Nếu bất kỳ dòng nào phía sau lỗi bất ngờ (thiết bị/ROM có hành vi lạ), lỗi sẽ giết chết cả
luồng tối ưu - và vì app có handler bắt lỗi toàn cục (`BoosterApp`), điều đó có thể **kill
luôn cả app giữa trận** một cách im lặng, không ai biết vì sao "tự nhiên mất HUD".

**Đã sửa:** bọc try/catch tổng quanh toàn bộ 1 lần lặp - nếu 1 lần đo lỗi, bỏ qua đúng lần
đó và tiếp tục bình thường ở lần sau, không để 1 lỗi hiếm gặp phá hỏng cả phần còn lại của
trận. Tách logic thành hàm `runWorkerIteration()` riêng để việc bọc bảo vệ rõ ràng, dễ kiểm
tra. Cũng sửa luôn để vòng lặp dừng ngay lập tức khi phát hiện người dùng đã thoát game
(trước đây có thể chờ thừa tới 20 giây do `stopSelf()` chạy không đồng bộ).

**Đã quét lại toàn bộ 15+ file .kt** bằng 3 công cụ tự động: cân bằng dấu ngoặc `{ }`, tìm
`return` sai vị trí trong hàm dạng biểu thức, và comment `/*` lồng không đóng - không còn
lỗi cú pháp nào sót lại trong toàn dự án tại thời điểm này.
