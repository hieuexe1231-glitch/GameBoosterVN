...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupClickListenersSafely() {
        ...
        binding.tabBtnOverview.setOnClickListener { showTab(TabId.OVERVIEW) }
        binding.tabBtnGame.setOnClickListener { showTab(TabId.GAME) }
        binding.tabBtnSystem.setOnClickListener { showTab(TabId.SYSTEM) }
        binding.tabBtnCleanup.setOnClickListener { showTab(TabId.CLEANUP) }
    }

    // ---------- THANH TAB DƯỚI CÙNG ----------

    private enum class TabId { OVERVIEW, GAME, SYSTEM, CLEANUP }
    private var currentTab = TabId.OVERVIEW

    private fun showTab(tab: TabId) {
        currentTab = tab
        binding.tabContentOverview.visibility = if (tab == TabId.OVERVIEW) android.view.View.VISIBLE else android.view.View.GONE
        binding.tabContentGame.visibility = if (tab == TabId.GAME) android.view.View.VISIBLE else android.view.View.GONE
        binding.tabContentSystem.visibility = if (tab == TabId.SYSTEM) android.view.View.VISIBLE else android.view.View.GONE
        binding.tabContentCleanup.visibility = if (tab == TabId.CLEANUP) android.view.View.VISIBLE else android.view.View.GONE

        val selectedBg = androidx.core.content.ContextCompat.getDrawable(this, R.drawable.btn_mode_selected)
        val pairs = listOf(
            Triple(binding.tabBtnOverview, binding.tabLabelOverview, tab == TabId.OVERVIEW),
            Triple(binding.tabBtnGame, binding.tabLabelGame, tab == TabId.GAME),
            Triple(binding.tabBtnSystem, binding.tabLabelSystem, tab == TabId.SYSTEM),
            Triple(binding.tabBtnCleanup, binding.tabLabelCleanup, tab == TabId.CLEANUP)
        )
        pairs.forEach { (container, label, selected) ->
            container.background = if (selected) selectedBg else null
            label.setTextColor(ContextCompat.getColor(this, if (selected) R.color.text_primary else R.color.text_secondary))
        }

        // Vào tab Game -> quét lại danh sách game thật đã cài (rẻ, nhanh, không cần cache
        // phức tạp - máy người dùng hiếm khi cài/gỡ game liên tục giữa các lần bấm tab).
        if (tab == TabId.GAME) populateDetectedGames()
    }

    // ---------- TỰ PHÁT HIỆN GAME ĐÃ CÀI ----------

    /** Phát hiện game THẬT đã cài trên máy qua PackageManager - không phải danh sách cứng.
     * SỬA THEO YÊU CẦU: giờ gộp CẢ Free Fire/Liên Quân vào chung 1 danh sách (không còn nút
     * riêng), và LỌC BỎ bàn phím/IME - lý do có lỗi thật trước đó (app bàn phím "Laban Key"
     * bị nhận nhầm thành game): field category của Android có thể mặc định về đúng giá trị
     * trùng với CATEGORY_GAME (0) ở một số app không khai báo rõ loại, không phải lỗi phát
     * hiện sai hoàn toàn - cần loại trừ thêm bằng danh sách IME đang cài (nguồn dữ liệu khác
     * hẳn, đáng tin hơn) để chắc chắn không lọt bàn phím vào danh sách game. */
    private fun detectInstalledGames(): List<android.content.pm.ApplicationInfo> {
        return try {
            val pm = packageManager
            val fallbackPrefixes = listOf(
                "com.tencent.ig", "com.pubg", "com.miHoYo", "com.mojang", "com.supercell",
                "com.riotgames", "com.activision", "com.ea.gp", "com.vng.", "com.garena.",
                "com.dts.", "net.gamestar", "com.gameloft"
            )
            // Danh sách bàn phím (IME) THẬT đang cài trên máy - lấy từ đúng nguồn dữ liệu
            // của hệ thống bàn phím, không suy đoán qua category như trên.
            val imePackages = try {
            ...
            pm.getInstalledApplications(PackageManager.GET_META_DATA)
                .filter { app ->
                    app.packageName != packageName &&
                        app.packageName !in imePackages &&
                        (app.flags and android.content.pm.ApplicationInfo.FLAG_SYSTEM) == 0 &&
                        pm.getLaunchIntentForPackage(app.packageName) != null &&
                        (
                            (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O &&
                                app.category == android.content.pm.ApplicationInfo.CATEGORY_GAME) ||
                                fallbackPrefixes.any { app.packageName.startsWith(it) } ||
                                app.packageName == FREE_FIRE_PACKAGE || app.packageName == LIEN_QUAN_PACKAGE
                        )
                }
                .sortedBy { pm.getApplicationLabel(it).toString() }
                .take(20) // giới hạn hợp lý, tránh danh sách quá dài nếu máy cài rất nhiều game
        } catch (e: Throwable) { emptyList() }
    }

    private fun populateDetectedGames() {
        val container = binding.detectedGamesContainer
        container.removeAllViews()
        val games = detectInstalledGames()
        binding.tvGamesCount.text = "${games.size} game"
        binding.tvNoGamesFound.visibility = if (games.isEmpty()) android.view.View.VISIBLE else android.view.View.GONE
        val pm = packageManager
        ...
        games.forEach { app ->
            val row = android.widget.LinearLayout(this).apply {
                ...
                layoutParams = lp
                background = androidx.core.content.ContextCompat.getDrawable(this@MainActivity, R.drawable.btn_secondary_redmagic)
            }

            val icon = android.widget.ImageView(this).apply {
                layoutParams = android.widget.LinearLayout.LayoutParams((40 * density).toInt(), (40 * density).toInt()).also {
                    it.marginEnd = (12 * density).toInt()
                }
                try { setImageDrawable(pm.getApplicationIcon(app)) } catch (_: Throwable) { }
            }

            val label = android.widget.TextView(this).apply {
                layoutParams = android.widget.LinearLayout.LayoutParams(0, android.widget.LinearLayout.LayoutParams.WRAP_CONTENT, 1f)
                text = try { pm.getApplicationLabel(app).toString() } catch (_: Throwable) { app.packageName }
                setTextColor(ContextCompat.getColor(this@MainActivity, R.color.text_primary))
                textSize = 14f
            }

            val boostBtn = android.widget.Button(this).apply {
                layoutParams = android.widget.LinearLayout.LayoutParams(
                    android.widget.LinearLayout.LayoutParams.WRAP_CONTENT, (40 * density).toInt()
                )
                text = "BOOST"
                isAllCaps = false
                textSize = 12f
                setTextColor(ContextCompat.getColor(this@MainActivity, R.color.text_primary))
                background = androidx.core.content.ContextCompat.getDrawable(this@MainActivity, R.drawable.btn_primary_redmagic)
                setPadding((16 * density).toInt(), 0, (16 * density).toInt(), 0)
                setOnClickListener {
                    val label = try { pm.getApplicationLabel(app).toString() } catch (_: Throwable) { app.packageName }
                    startGameMode(app.packageName, label)
                }
            }

            row.addView(icon)
            row.addView(label)
            row.addView(boostBtn)
    ...
    private fun clearOwnCache(showToast: Boolean) {
        // Cập nhật vòng tiến trình theo TỪNG BƯỚC THẬT đã xong (không phải hoạt ảnh giả) -
        // các lệnh shell bên dưới chỉ trả về xong/không xong, không có % tiến trình thật
        // sự, nên chia thành các mốc rõ ràng theo đúng số bước sẽ thực hiện.
        runOnUiThread {
            binding.gaugeCleanup.setValue(0f, "0%", "ĐANG DỌN")
            binding.tvCleanupStatus.text = "Đang xoá cache của app..."
        }
        Thread {
            var freed = 0L
            ...
            runOnUiThread {
                binding.gaugeCleanup.setValue(30f, "30%", "ĐANG DỌN")
                binding.tvCleanupStatus.text = "Đang dừng app nền..."
            }
            ...
            if (ShizukuHelper.hasPermission()) {
                ...
                runOnUiThread {
                    binding.gaugeCleanup.setValue(65f, "65%", "ĐANG DỌN")
                    binding.tvCleanupStatus.text = "Đang dọn cache toàn hệ thống..."
                }
            ...
            runOnUiThread {
                if (isFinishing || isDestroyed) return@runOnUiThread
                binding.gaugeCleanup.setValue(100f, "100%", "XONG")
                ...
                if (showToast) {
                    ...
                    binding.tvCleanupStatus.text = msg
                    log("Dọn cache: ${"%.2f".format(freedMb)} MB (booster) ${if (systemWideOk) "+ dừng $stoppedApps app nền + toàn hệ thống" else ""}")
                    Toast.makeText(this, msg, Toast.LENGTH_LONG).show()
                }
                refreshSystemInfo()
                // Về lại trạng thái sẵn sàng sau 2s để lần dọn kế tiếp không bị hiểu nhầm
                // là đang chạy tiếp từ 100%.
                mainHandler.postDelayed({
                    if (!isFinishing && !isDestroyed) {
                        binding.gaugeCleanup.setValue(0f, "0%", "SẴN SÀNG")
                        binding.tvCleanupStatus.text = "Sẵn sàng dọn dẹp"
                    }
                }, 2000)
            }
        }.start()
