    ...
    private fun reactToBottleneck(s: AdaptiveGameEngine.Snapshot, gamePackage: String) {
        when (s.bottleneck) {
            ...
            AdaptiveGameEngine.Bottleneck.MEMORY -> {
                // SỬA LỖI QUAN TRỌNG (nguyên nhân rất có thể gây "chơi được 1 lúc thì giật
                // lặp lại"): trước đây nhánh này gọi doLightCleanup() NGAY MỖI LẦN phát
                // hiện RAM áp lực, KHÔNG có giới hạn tần suất như nhánh "máy đang yên tĩnh"
                // (MIN_MAINTENANCE_GAP_MS). Nếu RAM cứ ở mức cao liên tục suốt một đoạn dài
                // của trận (rất hay xảy ra với game nặng, đặc biệt máy yếu) - dọn dẹp có
                // thể lặp lại mỗi 12-20 giây LIÊN TỤC, mỗi lần đều tốn CPU thật (nhiều lệnh
                // shell spawn qua Shizuku) - đúng kiểu giật lặp đi lặp lại người dùng mô tả.
                // Giờ dùng chung cơ chế giới hạn tần suất với nhánh yên tĩnh (chia sẻ
                // lastMaintenanceAt), nhưng cho phép khoảng cách NGẮN HƠN hẳn (60 giây thay
                // vì 4 phút) vì RAM áp lực thật vẫn cần xử lý khẩn hơn bảo trì định kỳ.
                stableCalmSamples = 0
                consecutiveThermalSamples = 0
                if (dimmedForThermal) restoreScreenBrightness()
                val now = SystemClock.elapsedRealtime()
                if (now - lastMaintenanceAt >= MIN_MEMORY_CLEANUP_GAP_MS) {
